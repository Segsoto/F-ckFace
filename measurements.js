(function () {
  ...
  function fields(category) {
    return category === "pantalones" ? pants : category === "mochilas" ? bags : clothing;
  }
  function editor(container, category, product = {}, original = product) {
    ...
    for (const [key, title] of [...selected, ...legacy]) {
      ...
      const editorTitle = key === "inseam_cm" ? "ANCHO " : title;
  ...
  function display(product) {
    ...
    const legacy = clothing.filter(([key]) => !selected.some(([field]) => field === key) && product[key] != null);
    for (const [key, title] of [...selected, ...legacy]) {
